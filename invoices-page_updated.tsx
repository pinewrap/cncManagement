...
export default function InvoicesPage() {
  ...
  // On-demand only — never runs on mount or on every filter change, since it
  // has to fetch and compute every matching invoice's tax-inclusive total,
  // not just whatever page happens to be loaded.
  async function handleShowSummary() {
    setSummaryLoading(true);
    const params = new URLSearchParams();
    params.set("summary", statusFilter);
    if (selectedCustomerId) params.set("customerId", selectedCustomerId);
  ...
  useEffect(() => {
    loadFirstPage();
    setSummary(null); // stale summary would be misleading once the filter changes
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            >
              {summaryLoading
                ? "Calculating..."
                : `Show ${statusFilter === "ALL" ? "total" : statusFilter.toLowerCase()} amount`}
            </button>
          </div>
        </div>

        {summary && (
          <div className="mt-3 inline-block rounded bg-brand/10 px-3 py-2 text-sm font-semibold text-brand-navy">
            {summary.count} {statusFilter === "ALL" ? "" : statusFilter.toLowerCase() + " "}
            invoice{summary.count !== 1 ? "s" : ""}
